...
| Script | Purpose |
| --- | --- |
| `data/tts.py` | Synthesizes `data/mock-call-transcript.txt` into `data/mock-call.wav` using **MAI Voice 2**, one voice per speaker, with stage directions rendered as real silence. |
| `data/stt.py` | Streams/sends that audio through three STT pipelines and scores each against the original transcript. Writes `data/stt-benchmark-results.json`. |

Run them with `uv run python data/tts.py` and `uv run python data/stt.py`.
...
505s synthetic support call, 989-word reference transcript. All three pipelines run
...
count - so variants 2 and 3 cost the same despite very different transports.
...
The benchmark is driven from the web UI: upload a call recording and, optionally, a
reference transcript, and the three architectures above run against it concurrently.
Word error rate is scored only when a reference transcript is supplied; latency and
transcripts are produced either way.
...
Uploaded audio in any format ffmpeg can read is normalized to 16-bit mono PCM; audio
already in that form is passed through untouched.
